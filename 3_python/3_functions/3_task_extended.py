...
def iterative_algorithm(amount, banknotes_values, banknotes_amount):
    ...
    results_by_lists = [list(combination) for combination in results]
    is_found = False
    print(results_by_lists)
    for combination in results_by_lists:
        for value in banknotes_values:
            if banknotes_amount.count(value) >= combination.count(value):
                is_found = True
            else:
                is_found = False
                break
